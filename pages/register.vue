<template>
  <section class="register-panel">
    ...
    <div v-else class="register">
      ...
      <form >
        <div class="wrapper">
          <a v-on:click="handleGithubSignIn">
          ...
          <a v-on:click="handleGoogleSignIn">
            <svg viewBox="0 0 128 128">
        ...
        <div class="wrapper">
          ...
          <!-- -->
          <button class="email-signup" v-on:click="handleEmailSignIn">Sign up</button>
        </div>
      </form>
    </div>
  </section>
...
if (data.value?.user !== null) {
  if (route.query.authSignup === 'true') {
    //@ts-expect-error
    await queryDatabase(data?.value?.user?.email);
  ...
  * {
  ...
  form {
    ...
    .email-signup {
      ...
      border: none;
      border-radius: 1rem;
      background-color: rgb(31, 115, 172);
      padding: 0rem;
      cursor: pointer;
    }
  }
